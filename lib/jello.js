var Jello = function (siteUrl, listOptions) {
    ...
    return {
        ...
        get: function (top, url) {
            ...
            if (self.filterObj.select || self.filterObj.where || self.filterObj.top)
            {
                ...
                if (self.filterObj.filter)
                    filter = (filter.length > 0) ? filter + "&" + self.filterObj.where : filter + "?" + self.filterObj.where;

                if (self.filterObj.top)
                    filter = (filter.length > 0) ? filter + "&" + self.filterObj.top : filter + "?" + self.filterObj.top;
